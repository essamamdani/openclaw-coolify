...
**Container:** ***REMOVED-DEPLOYMENT-ID***-111025847494  
...
docker exec ***REMOVED-DEPLOYMENT-ID***-111025847494 \
...
docker restart ***REMOVED-DEPLOYMENT-ID***-111025847494
...
### Check Configuration
```bash
ssh ***REMOVED-VPS*** "docker exec ***REMOVED-DEPLOYMENT-ID***-111025847494 \
  openclaw config get agents.defaults.sandbox.workspaceAccess"
# Expected: ro
```
...
ssh ***REMOVED-VPS*** "docker ps --format '{{.Names}}' | grep sbx"

# Check if workspace is mounted
ssh ***REMOVED-VPS*** "docker inspect <sandbox-container-name> --format '{{json .Mounts}}'"
...
```bash
# Find old sandbox
ssh ***REMOVED-VPS*** "docker ps -a | grep sbx"

# Stop and remove old sandbox
ssh ***REMOVED-VPS*** "docker stop <old-sandbox-name>"
ssh ***REMOVED-VPS*** "docker rm <old-sandbox-name>"
...
# Restart container
ssh ***REMOVED-VPS*** "docker restart ***REMOVED-DEPLOYMENT-ID***-111025847494"
```
